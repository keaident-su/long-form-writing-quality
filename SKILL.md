...
humanizer-zh 是**编辑/审阅阶段**的工具，不是**生成阶段**的工具。如果在生成时同时加载，它的规则会与本 Skill 直接冲突：

| humanizer-zh 规则 | 与本 Skill 的冲突 | 后果 |
|-------------------|-------------------|------|
| "变化节奏：短促有力的句子。然后是长句。混合使用" | 本 Skill 要求完整主谓宾、不碎片化 | **碎片化断句**（"裴砚，说"） |
| "允许一些混乱。完美的结构感觉像算法" | 本 Skill 要求逻辑严密、时间线递增 | **事件因果链错乱** |
| "两项优于三项" | 本 Skill 要求剧情结构完整 | 剧情结构残缺 |
| "删除金句：如果听起来像可引用的语句，重写它" | 本 Skill 要求对话自然有个性 | 对话平淡僵硬 |

**正确用法**：先加载 long-form-writing-quality 生成完整文本，**生成完成后**再单独加载 humanizer-zh 做润色。

### 低风险冲突：character-profile / coverage-report

这两个技能分别是"人物分析"和"审稿评价"工具，正常使用不干扰生成。但需要注意：

- **character-profile**：不要在写剧本时同时调用"生成人物小传"，否则每一场都会变成人物小传流水账
- **coverage-report**：不要在写剧本时同时调用"写审稿报告"，否则剧情会变成优缺点分析

### 无冲突：lark-doc

lark-doc 是文档操作工具，不影响内容质量。唯一注意：在线文档可能把长文本切成小段落导致空行偏多，但这是格式问题不是逻辑问题。
## 核心问题
...
| **事件因果链错乱（通用，v4.6.0新增）** | 时间戳递增但事件逻辑矛盾：船坞7月21日已靠帮修好，7月29日又写还在等船坞，8月2日又写船坞四十海里外——结果先于原因出现 | 事件必须按因果顺序推进：原因（等待船坞）→过程（船坞赶来）→结果（船坞到了）。同一事件的状态在所有章节中必须保持因果一致，不能先写结果再写原因 |
| **补充场事件状态与主场次矛盾（通用，v4.6.0新增）** | 主场次写船坞已经靠帮，补充场又写船坞还在三天航程外；主场次写人物已经离开，补充场又写正在送别 | 补充场的事件状态必须与主场次因果一致：如果主场次写事件已经发生，补充场不能又写事件还在进行中/还没发生 |
| **同一事件多状态矛盾（通用，v4.6.0新增）** | 同一事件（船坞到达）在不同章节中有三种状态：已到达/还在赶来/快到了，时间戳递增但状态矛盾 | 同一事件在所有章节中只能有一种状态描述；状态变化必须按时间顺序推进，不能倒退 || **事件逻辑顺序错乱（通用，v4.6.0新增）** | 时间戳是递增的（7月20日→7月21日→7月29日→8月2日），但事件发展阶段完全错乱：7月21日船坞已到并开始抢修，7月29日又变成"还在等三天航程"，8月2日又变成"距四十海里"——按道理应该是A→B→C→D→E，实际变成了A→C→B→D→F→E | 同一事件线（如"船坞救援""导弹攻击""逃亡路线"）的发展阶段必须严格按因果顺序推进：联系→等待→接近→到达→实施→完成。后发生的事件阶段不能倒退回更早的阶段 |
| **同一事件线阶段倒退（通用，v4.6.0新增）** | 船坞救援线：7月21日已靠帮抢修（阶段：到达），7月29日又变成"还有三天航程"（阶段：等待），8月2日又变成"四十海里"（阶段：接近）——已经到了又倒回去等 | 同一事件线的发展阶段只能前进不能倒退。如果主场次已经写"到达"，后续所有补充场不能再写"还在等待/还在路上" |
| **事件因果关系倒置（通用，v4.6.0新增）** | 先写"船靠港了"，后面又写"等船坞来修船"——靠港了怎么还在等船坞？因果关系完全颠倒 | 事件必须有合理的因果顺序：原因在前，结果在后。不能先写结果再写原因，导致逻辑上说不通 || **主场次与补充场事件状态矛盾（v4.4.0新增）** | 181场（7月8日13:00）写小晚已在机场离开，但181补2（7月9日21:30）又写送她上晚上九点的航班 | 补充场的事件状态必须与主场次一致：如果主场次说人已经走了，补充场不能又写送她走；补充场时间戳必须在主场次之前或之后，不能出现人已经走了又正在送的矛盾 |
...
25. 缺少事件因果链一致性校验：时间戳虽然递增，但事件的因果关系被打乱——先写结果（船坞到了），再写原因（船坞还在赶来），导致逻辑自相矛盾（v4.6.0新增）
26. 缺少补充场事件状态一致性校验：补充场的事件状态与主场次矛盾，主场次说事件已发生，补充场又说还在进行中（v4.6.0新增）
27. 缺少同一事件多状态矛盾检测：同一事件在不同章节中有多种状态描述，时间戳递增但状态矛盾（v4.6.0新增）25. 缺少事件逻辑顺序校验：只校验了时间戳单调递增，没有校验同一事件线的发展阶段是否按因果顺序推进，导致"船坞7月21日已到、7月29日又在等"的阶段倒退（v4.6.0新增）
26. 缺少同一事件线阶段一致性校验：同一事件线（救援/攻击/逃亡/建设等）的发展阶段只能前进不能倒退，主场次已写"到达"，后续补充场不能再写"还在等待"（v4.6.0新增）
27. 缺少事件因果关系校验：事件必须有合理的因果顺序，不能出现"先靠港了又等船坞来修"的因果倒置（v4.6.0新增）
## 工作流（必须严格遵守）
...
21. **建立全局事件因果链表**（v4.6.0新增）：列出所有关键事件及其因果顺序（原因→过程→结果），确保按因果顺序推进；每个事件只写一次"发生"的节点，后续章节只能写事件的后续影响，不能重复写事件本身
22. **建立事件状态时间线表**（v4.6.0新增）：记录每个关键事件在不同时间点的状态（如：等待中→赶来中→已到达→已修好），确保时间戳递增时事件状态也按因果顺序推进，不能倒退
21. **建立全局事件线发展阶段表**（v4.6.0新增）：为每一条关键事件线（如"船坞救援""导弹攻击""逃亡路线""建设扩张"等）列出发展阶段（联系→等待→接近→到达→实施→完成），在拆解阶段就明确每个阶段对应哪些场次，确保阶段只能前进不能倒退
22. **建立事件因果关系校验表**（v4.6.0新增）：列出所有关键事件的因果链（原因→结果），确保生成时原因在前、结果在后，不能因果倒置
### 第二步：分批生成
...
30. **同一人物离开时间必须统一**（v4.4.0新增）：同一人物的离开时间（航班/车次时间）在所有场次中必须一致；如有变动，必须有明确的情节说明（如"航班取消改签到明天"）。
31. **事件因果顺序必须正确**（v4.6.0新增）：事件必须按因果顺序推进（原因→过程→结果），不能先写结果再写原因。例如：不能先写船坞已经靠帮了，再写船坞还在赶来的路上。
32. **补充场事件状态必须与主场次一致**（v4.6.0新增）：补充场的事件状态必须与主场次因果一致。如果主场次写事件已经发生（船坞已到达），补充场不能再写事件还在进行中（船坞还在赶来）。
33. **同一事件状态不能倒退**（v4.6.0新增）：同一事件的状态必须随时间推进而向前发展，不能倒退。例如：船坞"已到达"后不能再变回"还在赶来"。31. **事件发展阶段只能前进不能倒退**（v4.6.0新增）：同一事件线的发展阶段（联系→等待→接近→到达→实施→完成）只能前进不能倒退。主场次已经写了"到达"，后续补充场不能再写"还在等待/还在路上"。
32. **事件因果关系必须合理**（v4.6.0新增）：事件必须有合理的因果顺序，原因在前、结果在后。不能出现"先靠港了又等船坞来修""已经打完仗了又在布置防线"的因果倒置。
### 第三步：每批质量检查（闸门）
...
- [ ] **日本9月表述检查**（v4.1.0新增）：9月日本校园场景是否误写"新学期/新学年开学"，应为"暑假结束/前期末/後期即将开始"
- [ ] **事件因果链一致性检查**（v4.6.0新增）：本场事件状态是否与前面章节的事件状态因果一致，有没有先写结果再写原因
- [ ] **补充场事件状态一致性检查**（v4.6.0新增）：本场（补充场）的事件状态是否与对应主场次一致，有没有主场次说已发生、补充场说还在进行中
- [ ] **同一事件状态倒退检查**（v4.6.0新增）：同一事件的状态是否随时间推进向前发展，有没有倒退（如船坞已到达又变回还在赶来）- [ ] **人物年龄精确到生日检查**（v4.2.0新增）：本场人物年龄是否根据精确出生日期和场景日期计算，未到生日是否误增岁
...
### v4.6.0
- 新增**事件因果链一致性检测**（通用）：时间戳虽然递增，但事件的因果关系被打乱——先写结果（船坞到了），再写原因（船坞还在赶来），导致逻辑自相矛盾。
- 新增**补充场事件状态与主场次矛盾检测**（通用）：主场次写事件已发生，补充场又写事件还在进行中，导致因果顺序错乱。
- 新增**同一事件多状态矛盾检测**（通用）：同一事件在不同章节中有多种状态描述，时间戳递增但状态矛盾（如船坞7月21日已到、7月29日还在等、8月2日又快到了）。
- 新增任务拆解阶段的2张全局表：事件因果链表、事件状态时间线表。
- 同步工作流闸门（第31—33条）与质量检查清单（事件因果链/补充场一致性/状态倒退）。
### v4.5.0
