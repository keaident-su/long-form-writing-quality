...
**长文本生成的核心问题：单次上下文装不下全部已写内容，写到后面就忘了前面。**
**解决方案：不靠记忆，靠工程化手段兜住——状态基线表 + 自动校验脚本 + 全量扫描。**

### 核心原则

1. **不靠记忆，靠状态**：所有关键状态（场次时间、人物位置、车辆可用性、关键情节节点）都必须落盘成表格，每批生成时对照表格，不依赖模型记忆
2. **不靠人眼，靠脚本**：每批写完必须自动跑校验脚本，脚本不通过就不交付，不能靠人眼抽查
3. **不靠补丁，靠闭环**：发现问题后，先全量扫描列出所有同类问题，一次性修完，再跑校验，不是指出哪修哪

### 措施1：开工前必须落盘"全局状态基线表"

任何一幕开工前，先写一个 ctN_state_baseline.md，包含以下表格，**不写完表格不许写正文**：

| 表格名称 | 内容 | 校验方式 |
|---------|------|----------|
| **场次编号表** | 场号、时间戳、地点、出场人物、大纲锚点 | 正文任何时间戳必须能在表中找到对应行 |
| **人物位置表** | 每个人物在每场结束时所在位置 | 正文提到某人物时，检查该时间点他是否在正确位置 |
| **车辆可用性表** | 每台车在哪个时间点可驾驶/在改装/已报废/已被借走 | 正文提到某车时，检查该时间点车是否可用 |
| **关键情节节点表** | "第一次XX"必须唯一、购车/改装/报废时间点必须唯一 | 正文出现"第一次XX"时，检查全文是否只出现一次 |
| **集数编号表** | 集号、起止场号、时间线范围 | 集数编号必须连续不重复 |

### 措施2：每批写完强制跑自动校验

每次写完一批（约3场，≤4000字），立刻跑自动校验脚本，**脚本不过就不交付**：

- ✅ 时间线单调性检查（datetime比较，前一场时间必须≤后一场）
- ✅ 场次四字段完整性检查（大纲锚点/时间/地点/出场人物缺一即报错）
- ✅ 补充场位置检查（补充场必须紧跟同号主场次）
- ✅ 集数标注连续性检查
- ✅ 车辆可用性检查（正文提到某车时，检查该时间点车是否可用）
- ✅ 人物位置检查（正文提到某人物时，检查该时间点他是否在正确位置）

### 措施3：补充场时间戳硬规则

写补充场时，必须严格遵守：

1. 补充场时间必须**严格在同号主场次时间之后**
2. 补充场时间必须**严格在下一个主场次时间之前**
3. 补充场内容必须**提供新信息**（不同人物视角/新发现/情节转折），不能重复主场次已写的情节
4. 补充场事件状态必须**与主场次因果一致**（主场次写"已到达"，补充场不能写"还在赶来"）

### 措施4：交付前必须跑全量扫描

每次交付Word/长文档前，跑全量扫描脚本，自动检测：

- ✅ 所有"XX年前/后"的相对时间表述，跟实际出生日期/事件日期交叉核对
- ✅ 所有车辆名称出现的位置，跟车辆可用性表核对
- ✅ 所有"第一次"类表述，确认全文只出现一次
- ✅ 所有人物年龄，跟出生日期×场景日期计算核对
- ✅ 所有场次时间戳严格单调递增
- ✅ 所有补充场紧跟同号主场次

### 措施5：收到反馈后先全量扫描再修，不是指出哪修哪

收到用户反馈"某部分逻辑不通"时，**不要立刻猜一个问题修一个**，必须：

1. 导出全片场次索引（所有场号+时间戳+地点+人物）
2. 逐场比对时间线/车辆/人物位置/关键情节节点
3. 列出全部矛盾清单（同类问题一次性列全）
4. 一次性修完全部问题
5. 再跑校验脚本确认全部通过
6. 最后交付

**正确示例**：发现"船坞7月21日已到、7月29日又在等"时，不是只修这一场，而是把所有"船坞"相关场次全部找出来，按因果顺序重新排序，一次性修好。
## 工作流（必须严格遵守）
...
### v5.0.0
- **工程化防退化体系**：从"靠记忆/靠人眼"升级为"靠状态基线表+自动校验脚本+全量扫描"的闭环体系。
- 新增**全局状态基线表**：开工前必须落盘5张表（场次编号/人物位置/车辆可用性/关键情节节点/集数编号），不写完表格不许写正文。
- 新增**每批强制自动校验**：每批写完必须跑脚本，脚本不过就不交付。
- 新增**补充场时间戳硬规则**：补充场时间必须在主场次之后、下一场之前，且必须提供新信息。
- 新增**交付前全量扫描**：所有相对时间表述、车辆可用性、"第一次"类表述、人物年龄必须全量核对。
- 新增**反馈修复流程**：收到反馈后先全量扫描列出所有同类问题，一次性修完，再跑校验，不是指出哪修哪。
- 核心原则：不靠记忆，靠状态；不靠人眼，靠脚本；不靠补丁，靠闭环。
### v4.6.0
